...
export class DevWorkspaceOperatorInstaller implements Installer {
  ...
  getDeployTasks(): Listr.ListrTask<any> {
    console.log('DevWorkspaceOperatorInstaller ---> getDeployTasks ' + this.skip)
    return {
      title: `Install ${DevWorkspace.PRODUCT_NAME} operator`,
      skip: () => {
        console.log('DevWorkspaceOperatorInstaller ---> ' + this.skip)
        return this.skip
      },
      task: async (ctx: any, _task: any) => {
        const tasks = newListr()
        console.log('DevWorkspaceOperatorInstaller ---> ' + ctx[DevWorkspaceContext.NAMESPACE])
