...
export class DexInstaller implements Installer {
        ...
        tasks.add({
          ...
          task: async (_ctx: any, task: any) => {
            ...
            if (await this.kubeClient.isSecretExists(DexInstaller.TLS_SECRET_NAME, DexInstaller.NAMESPACE_NAME)) {
              ...
              const _localHost1 = 'localhost'
              const _localHost2 = '127.0.0.1'
              //david
              certificate.spec.dnsNames = [dexDomain, wildCardDexDomain, _localHost1, _localHost2]
  ...
  getDexResourceFilePath(fileName: string): string {
    var _path = getEmbeddedTemplatesDirectory()
    var _filePath = path.join(getEmbeddedTemplatesDirectory(), '..', 'resources', 'dex', fileName)
    console.log("_path = " + _path + "<------> _filePath = " + _filePath);  
